      ...
      <div className="grid-2" style={{ marginBottom: 12 }}>
        ...
        <div className="card">
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            <button onClick={() => { setTgConnected(true); setTgTestSent(false); }} style={{
              ...
              color: tgConnected ? '#06d6a0' : '#fff', fontSize: 12, fontWeight: 700, cursor: 'pointer'
            }}>
              {tgConnected ? '✅ Conectado' : 'Conectar Bot'}
            </button>
            <button onClick={() => setTgTestSent(true)} disabled={!tgConnected} style={{
              ...
              fontSize: 12, fontWeight: 600, cursor: tgConnected ? 'pointer' : 'not-allowed'
            }}>
          ...
          {!tgConnected && (
            <div style={{ marginTop: 12, padding: 10, background: '#020617', borderRadius: 8, fontSize: 10, color: '#94a3b8', lineHeight: 1.6 }}>
              ...
              4. Envía un mensaje a tu bot, luego busca tu Chat ID en <code>api.telegram.org/bot{'<TOKEN>'}/getUpdates</code>
            </div>
          )}
        </div>
        ...
        <div className="card">
          ...
          {[
            { key: 'market', icon: '🌡️', label: 'Alertas del mercado', desc: 'Te avisamos cuando hay una oportunidad de compra en el mercado' },
            { key: 'dollar', icon: '💵', label: 'Alertas del dólar', desc: 'Te avisamos cuando el dólar está barato o muy caro' },
            { key: 'stocks', icon: '📊', label: 'Alertas de tus acciones', desc: 'Te avisamos cuando alguna de tus acciones baja mucho (oportunidad)' },
            { key: 'daily', icon: '📋', label: 'Resumen diario (18:00)', desc: 'Un resumen con todo lo que pasó hoy en tus inversiones' },
          ].map(a => (
            <div key={a.key} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 0', borderBottom: '1px solid #0f172a' }}>
              ...
              <button onClick={() => setTgAlerts({ ...tgAlerts, [a.key]: !tgAlerts[a.key as keyof typeof tgAlerts] })}
